...
sudo -H -u $caller_user bash -c 'pip install --upgrade pip'
sudo -H -u $caller_user bash -c 'pip install git-review virtualenv'

if [ $(IsXenial) == True ]; then
  echo 'Setting up PY3'
  apt-get install -y python3-dev python3-pip
  sudo -H -u $caller_user bash -c 'pip3 install --upgrade pip3'
  sudo -H -u $caller_user bash -c 'pip3 install git-review virtualenv'
fi
...
if [[ ! -z "${_ORIGINAL_PROXY}" ]]; then
  prx=$(echo "${_ORIGINAL_PROXY}" | awk -F '//' '{print $2}' | 
        awk -F ':' '{print $1}')
  cfgFile="/home/$caller_user/.ssh/config"
  echo "Host *" >> $cfgFile
  echo "ProxyCommand nc -X 5 -x $prx:1080 %h %p" >> $cfgFile
fi
